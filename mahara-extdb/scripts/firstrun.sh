...
if [ ! -f /var/www/html/mahara/config.php ]; then
	...
	sed -i "s|dbuser   = ''|dbuser   = '$MAHARA_DB'|g" /var/www/html/mahara/config.php
  	sed -i "s|dbpass   = ''|dbpass   = '$MAHARA_PASSWORD'|g" /var/www/html/mahara/config.php
        sed -i "s|dbhost   = 'localhost'|dbhost   = 'db'|g" /var/www/html/mahara/config.php
  	...
  	mysql -u "$DB_ENV_USER" --password="$DB_ENV_PASS" -h "$DB_PORT_3306_TCP_ADDR" -P "$DB_PORT_3306_TCP_PORT" < /tmp/mahara.sql
	echo " " && echo " --- " && echo " "
